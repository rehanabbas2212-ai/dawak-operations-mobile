...
function renderCashSearch(r){
  ...
  if(editRecord){
    ...
    const deleteButton=state.canDeleteAwbs&&editRecord.deletable?'<button class="danger" onclick="deleteCashAwb()">Delete AWB</button>':'';
    if(editRecord.bag_id&&state.canDeleteAwbs){
      const count=Number(editRecord.bag_awb_count||0);
      historyAction=`<button class="danger bag-delete history-delete-button" onclick="deleteWholeCashBag()">Delete Whole Bag${count?` (${count} AWB${count===1?'':'s'})`:''}</button>`;
      deleteNote=`<p class="muted admin-note">This AWB is inside <strong>${esc(editRecord.bag_name||'a transport bag')}</strong>. To keep the custody records consistent, delete the whole bag instead of deleting only this AWB.</p>`;
    }
...
window.deleteCashAwb=async()=>{const r=state.cashEditRecord;if(!state.canDeleteAwbs||!r||!r.deletable)return;const reason=prompt(`Reason for permanently deleting ${r.awb}:`);if(!reason?.trim())return;if(!confirm(`Permanently delete standalone AWB ${r.awb} and all of its AWB history? This cannot be undone.`))return;busy(true);try{await api('/rest/v1/rpc/delete_payment_line',{method:'POST',body:{p_payment_line:r.id,p_reason:reason.trim()}});state.cashEditRecord=null;$('cashSearchResult').innerHTML='';$('cashSearchResult').classList.add('hidden');$('cashSearchAwb').value='';notice('cashSearchStatus','AWB deleted. The deletion reason remains in the admin audit log.',true);await Promise.all([loadAvailableAwbs(),refreshCash()])}catch(e){notice('cashSearchStatus',e.message)}finally{busy(false)}};
...
window.deleteWholeCashBag=async()=>{
  const r=state.cashEditRecord;
  if(!state.canDeleteAwbs||!r?.bag_id)return;
  const bagName=String(r.bag_name||'').trim();
  const awbCount=Number(r.bag_awb_count||0);
  const legCount=Number(r.bag_leg_count||1);
...
${bagName}`);
  if(String(typed||'').trim()!==bagName){notice('cashSearchStatus','Whole-bag deletion cancelled because the bag name did not match.');return;}
  if(!confirm(`Permanently delete bag ${bagName}${legCount>1?` (${legCount} journey legs)`:''}, all ${awbCount} AWBs, and all custody history? This cannot be undone.`))return;
  busy(true);
  try{
    const result=await api('/rest/v1/rpc/delete_whole_cash_bag',{method:'POST',body:{p_batch:r.bag_id,p_reason:reason.trim(),p_confirm_name:bagName}});
    let proofMessage='';
    ...
    state.cashEditRecord=null;
    $('cashSearchResult').innerHTML='';$('cashSearchResult').classList.add('hidden');$('cashSearchAwb').value='';
    notice('cashSearchStatus',`Whole bag deleted: ${Number(result?.deleted_batches||0)} bag leg(s), ${Number(result?.deleted_awbs||0)} AWB(s).${proofMessage}`,!proofMessage.includes('could not'));
...
// OWNER-ONLY STANDALONE AWB CLEANUP
...
$('deleteSelectedTestAwbs').onclick=async()=>{
  ...
  notice('testDeleteStatus','');
  if(!ids.length){notice('testDeleteStatus','Select at least one standalone AWB.');return}
  if(reason.length<3){notice('testDeleteStatus','Enter a deletion reason.');return}
  if(!confirm(`Delete ${ids.length} selected standalone AWB${ids.length===1?'':'s'}? This cannot be undone.`))return;
  busy(true);
  try{
    const result=await api('/rest/v1/rpc/delete_my_test_awbs',{method:'POST',body:{p_payment_lines:ids,p_reason:reason}});
    notice('testDeleteStatus',`${result?.deleted_count||0} standalone AWB(s) deleted.${result?.skipped_count?` ${result.skipped_count} skipped because they were no longer eligible.`:''}`,true);
    state.myTestAwbs=await api('/rest/v1/rpc/list_my_deletable_awbs',{method:'POST',body:{}})||[];
    renderMyTestAwbs();
    await Promise.all([loadAvailableAwbs(),refreshCash()]);
...
function renderBatch(b){let action='—';if(state.me.role!=='viewer'&&b.pending_to===state.me.id)action=`<div class="action-stack">${proofInput(b.id,'accept')}<button onclick="acceptBag('${b.id}')">Accept whole bag</button></div>`;else if(state.me.role!=='viewer'&&b.current_custodian===state.me.id&&['OPEN','IN_TRANSIT'].includes(b.status))action=`<div class="action-stack"><select id="to-${b.id}"><option value="">Loading recipients…</option></select><input id="seal-${b.id}" placeholder="Seal / bag">${proofInput(b.id,'handover')}<button onclick="handoverBag('${b.id}')">Hand over whole bag</button></div>`;else if(['coordinator','hub_leader'].includes(state.me.role)&&b.current_custodian===state.me.id&&['ARRIVED','EXCEPTION'].includes(b.status))action=`<button onclick="showArrival('${b.id}')">Check AWBs individually</button>`;return `<tr><td>${esc(b.batch_name)}</td><td>${esc(hubFor(b.origin_hub_id))} → ${esc(hubFor(b.destination_hub_id))}<br><small>Next checkpoint</small></td><td>${Number(b.item_count||0)}<br><small>${Number(b.cash_item_count||0)} CASH / ${Number(b.card_item_count||0)} CARD</small></td><td>AED ${Number(b.expected_amount||0).toFixed(2)}</td><td>${Number(b.card_item_count||0)}<br><small>AED ${Number(b.card_amount||0).toFixed(2)} reference</small></td><td><span class="pill">${esc(b.status)}</span></td><td>${esc(nameFor(b.current_custodian))}<br><small>${esc(hubFor(b.current_hub_id))}</small></td><td>${esc(nameFor(b.pending_to))}</td><td>${action}</td><td><button class="secondary" onclick="showAudit('${b.id}')">History & AWBs</button></td></tr>`}
...
function renderAwbHistory(item){const history=item.history||[];return `<details class="awb-history"><summary><span><strong><span class="payment-badge">${esc(item.payment_type)}</span> ${esc(item.awb)}</strong><small>AED ${Number(item.amount||0).toFixed(2)} • Final: ${esc(hubFor(item.final_destination_hub_id))} • Collected by: ${esc(nameFor(item.collected_by))}</small></span><span class="pill">${esc(item.processing_result||item.cash_status||'IN BAG')}</span></summary><div class="awb-events">${history.length?history.map(x=>`<div class="audit-item compact"><strong>${esc(formatDate(x.created_at))}</strong><div>${esc(eventLabel(x.event_type))}<br><small>${esc(nameFor(x.from_user_id))} → ${esc(nameFor(x.to_user_id))} • ${esc(hubFor(x.from_hub_id))} → ${esc(hubFor(x.to_hub_id))}${x.notes?` • ${esc(x.notes)}`:''}</small></div></div>`).join(''):'<p class="muted">No individual movement events recorded.</p>'}</div></details>`}
...
window.showAudit=async id=>{try{
  ...
  $('auditPanel').innerHTML=`<div class="section-heading"><div><p class="eyebrow">FULL BAG AUDIT</p><h3>${esc(b.batch_name||'Transport bag')} history</h3></div>${addPhoto}</div><div class="history-summary"><div><span>Total AWBs</span><strong>${items.length}</strong></div><div><span>CASH</span><strong>${items.filter(x=>x.payment_type==='CASH').length} • AED ${Number(b.expected_amount||0).toFixed(2)}</strong></div><div><span>CARD</span><strong>${items.filter(x=>x.payment_type==='CARD').length} • AED ${Number(b.card_amount||0).toFixed(2)}</strong></div><div><span>Status</span><strong>${esc(b.status||'—')}</strong></div></div><h3>Tracking bills in this bag</h3><p class="muted">Open any AWB to see its individual custody trail.</p><div class="awb-history-list">${awbList}</div><h3 class="timeline-title">Bag custody timeline</h3>${timeline}`;
  $('auditPanel').scrollIntoView({behavior:'smooth',block:'start'});
}catch(e){notice('cashStatus',e.message)}};
